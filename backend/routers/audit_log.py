...
from services import audit_log as svc
from schemas import audit_log as schema

router = APIRouter(prefix="/api/audit_log", tags=["AuditLog"])


@router.get("/", response_model=list[schema.AuditLogResponse])
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=schema.AuditLogResponse)
def create_item(payload: schema.AuditLogCreate, db: Session = Depends(get_db)):
    return svc.create(db, payload.dict())


@router.put("/{id}")
def update_item(id: Any, payload: schema.AuditLogUpdate, db: Session = Depends(get_db)):
