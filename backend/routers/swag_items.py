...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=schema.SwagItemResponse)
def create_item(payload: schema.SwagItemCreate, db: Session = Depends(get_db)):
    return svc.create(db, payload.dict())


@router.put("/{id}")
def update_item(id: Any, payload: schema.SwagItemUpdate, db: Session = Depends(get_db)):
